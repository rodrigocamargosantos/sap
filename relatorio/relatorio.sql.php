...
class RelatorioSQL
{
	...
	public function relatorioCompetenciaSql()
	{
		...
		$MKInicio	= mktime(0, 0, 0, date('n')-1, 1, date('Y'));
		$MKFim		= mktime(0, 0, 0, date('n'), 1, date('Y'));
	...
	public function conteudoModalSql()
    {
		...
		$Sql = "SELECT pro.id_processamento, 
					   ...
					   to_char(pro.dt_previsao_recebimento_arquivo, 'DD/MM/YYYY')AS dt_previsao_recebimento_arquivo, 
    ...
    {
        $Sql = "SELECT pro.id_processamento, LPAD(mes || '/' || ano, 7, '0')AS competencia, con.sigla_convenio || ' - ' || con.descricao AS convenio, 
